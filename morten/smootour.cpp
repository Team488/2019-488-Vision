...
int main(int argc, const char * argv[])
{
    ...
    cv::namedWindow("smootour-test");
    namedWindow("implicit");
    namedWindow("smooth");
    ...
    while(running) {
        ...
        cv::cvtColor(thresholded, thresholdedFrame, COLOR_GRAY2RGB);
        
        //plain contours
        std::vector<std::vector<cv::Point> > contours;
        cv::Mat thresholded_contours = thresholded.clone();
        cv::findContours(thresholded_contours, contours, RETR_TREE, CHAIN_APPROX_NONE );
        ...
        cv::cvtColor(implicit_image, implicitFrame, COLOR_GRAY2RGB);
        ...
        cv::imshow("implicit", implicit_image);
